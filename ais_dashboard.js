const NS = {
  "0":{ l:"Under Way (Engine)", c:"underway" }, "1":{ l:"At Anchor",          c:"anchor"   },
  "2":{ l:"Not Under Command",  c:"other"    }, "3":{ l:"Restricted Manoeuvr", c:"other"    },
  "4":{ l:"Constrained Draft",  c:"other"    }, "5":{ l:"Moored",              c:"moored"   },
  "6":{ l:"Aground",            c:"other"    }, "7":{ l:"Engaged Fishing",     c:"underway" },
  "8":{ l:"Under Way (Sail)",   c:"underway" }, "9":{ l:"HSC",                 c:"underway" },
  "10":{ l:"WIG", c:"underway" }, "11":{ l:"Power-driven Tow", c:"underway" },
  "12":{ l:"Towing Alongside", c:"underway" }, "13":{ l:"Reserved", c:"other" },
  "14":{ l:"AIS-SART", c:"other" }, "15":{ l:"Not Defined", c:"other" },
};
const nsInfo = code => NS[String(code)] || { l:"Unknown", c:"other" };
const CLR = { underway:"#38b6e8", moored:"#f5a623", anchor:"#9b7dea", other:"#4d7a9e" };
...
function resizeCanvas() { canvasW = vc.width = window.innerWidth; canvasH = vc.height = window.innerHeight; redrawCanvas(); }
window.addEventListener("resize", resizeCanvas);

function drawVessel(px, py, angleDeg, cls, alpha, highlight) {
  const z = map.getZoom();
  const sz = z<=3?3:z<=5?4:z<=7?5:z<=9?6:8;
  const clr = CLR[cls];
  ctx.save();
  ctx.translate(px, py);
  ...
  ctx.beginPath();
  if (cls === "underway") {
    ctx.moveTo(0,-sz*1.5); ctx.lineTo(sz*.8,sz*.8); ctx.lineTo(-sz*.8,sz*.8); ctx.closePath();
  } else if (cls === "moored") {
    ctx.rect(-sz*.75,-sz*.75,sz*1.5,sz*1.5);
  } else if (cls === "anchor") {
    ctx.moveTo(0,sz*1.3); ctx.lineTo(sz*.8,-sz*.7); ctx.lineTo(-sz*.8,-sz*.7); ctx.closePath();
  } else {
    ctx.arc(0,0,sz*.75,0,Math.PI*2);
  }
  ctx.fillStyle = highlight ? "#fff" : clr;
  ctx.fill();
  if (highlight && cls !== "other") { ctx.strokeStyle = clr; ctx.lineWidth = 1.5; ctx.stroke(); }
  ctx.restore();
}
...
function redrawCanvas() {
  ...
  const z = map.getZoom(), alpha = z <= 3 ? 0.72 : 0.88;
...
// ── HELPERS ──────────────────────────────────────────────────────────
function toggleLeft() { leftCollapsed = !leftCollapsed; document.getElementById("left-panel").classList.toggle("collapsed", leftCollapsed); }
function fitAll() { if (allBounds) map.fitBounds(allBounds, { padding:[10,10] }); }
function setProgress(p, t) { document.getElementById("lbar").style.width = p+"%"; if (t) document.getElementById("lsub").textContent = t; }
...
function copyCoords() {
  ...
  coordCopyTimer = setTimeout(() => document.querySelectorAll(".coord-chip").forEach(el => el.classList.remove("copied")), 1500);
}
...
function showTip(f, ev) {
  const p = f.properties, [lng, lat] = f.geometry.coordinates, ni = nsInfo(p.navstat);
  ...
  document.getElementById("ht-hdg").textContent = (p.heading != null && +p.heading !== 511) ? (+p.heading)+"°" : "COG "+(+p.course||0).toFixed(1)+"°";
...
function hideTip() { htip.classList.remove("on"); }
function posTip(ev) {
  const tw = htip.offsetWidth||180, th = htip.offsetHeight||120;
  let x = ev.clientX+16, y = ev.clientY-th/2;
...
map.on("mousemove", e => {
  ...
  if (best !== hoverIdx) { hoverIdx = best; best >= 0 ? showTip(allVessels[best], e.originalEvent) : hideTip(); }
  else if (best >= 0) posTip(e.originalEvent);
});
...
map.on("click", e => {
  ...
  if (best >= 0) {
    selectedIdx = best; selectedFeature = allVessels[best];
    const [lng, lat] = allVessels[best].geometry.coordinates;
    map.flyTo([lat, lng], Math.max(map.getZoom(), 12), { duration:0.7 });
    showDetail(allVessels[best]); renderList(false); redrawCanvas();
  }
});
...
function loadGeoJSON(geojson) {
  ...
  for (let i = 0; i < allVessels.length; i++) {
    ...
    if (c==="underway") uw++; else if (c==="moored") mo++; else if (c==="anchor") an++;
  }
...
function applyFilter() {
  const q = searchQuery.toLowerCase(); filteredCache = [];
  for (let i = 0; i < allVessels.length; i++) {
...
function renderList(reset) {
  ...
  if (!filteredCache.length) { inner.style.height="60px"; vp.innerHTML='<div class="no-data" style="position:static">No vessels match</div>'; return; }
  inner.style.height = (filteredCache.length * ROW_H)+"px";
  if (reset) { outer.scrollTop=0; vsScroll=0; } else vsScroll=outer.scrollTop;
  ...
  for (let vi = si; vi <= ei; vi++) {
    const fi = filteredCache[vi], f = allVessels[fi], p = f.properties;
    ...
    el.style.top = (vi*ROW_H)+"px"; el.dataset.fi = fi;
...
<div class="vi-dest">${dest?`→ <span>${dest}</span>`:'<span style="color:var(--text3)">No destination</span>'}</div>`;
    el.addEventListener("click", () => {
      ...
      selectedIdx=fIdx; selectedFeature=allVessels[fIdx];
      const [lng,lat] = allVessels[fIdx].geometry.coordinates;
      map.flyTo([lat,lng], Math.max(map.getZoom(),12), {duration:.7});
      showDetail(allVessels[fIdx]); renderList(false); redrawCanvas();
    });
    frag.appendChild(el);
  }
...
function closeDetail() {
  selectedIdx=-1; selectedFeature=null;
  document.getElementById("right-panel").classList.remove("open");
  renderList(false); redrawCanvas();
}
function showDetail(f) {
  const p=f.properties, [lng,lat]=f.geometry.coordinates, ni=nsInfo(p.navstat);
  const color=CLR[ni.c], speed=p.speed!=null?+p.speed:null, course=p.course!=null?+p.course:null;
  const hv=p.heading!=null&&+p.heading!==511, heading=hv?+p.heading:(course||0);
...
function startDraw() {
  drawMode=true; map.getContainer().style.cursor="crosshair";
  const btn=document.getElementById("draw-btn");
  btn.style.background="rgba(56,182,232,0.2)"; btn.style.borderColor="var(--sky)";
}

map.on("mousedown", e => {
  if (!drawMode) return;
  drawStart=e.latlng; drawEnd=e.latlng;
  if (tempRect) { map.removeLayer(tempRect); tempRect=null; }
  tempRect=L.rectangle([drawStart,drawEnd],{color:"#38b6e8",weight:1.5,fillColor:"#38b6e8",fillOpacity:.08,dashArray:"5 4"}).addTo(map);
  map.dragging.disable(); L.DomEvent.stopPropagation(e);
});
map.on("mousemove", e => { if (drawMode && drawStart) { drawEnd=e.latlng; if(tempRect) tempRect.setBounds([drawStart,drawEnd]); } });
map.on("mouseup", e => {
  if (!drawMode||!drawStart) return;
  drawEnd=e.latlng; map.dragging.enable(); drawMode=false;
  map.getContainer().style.cursor="";
  const btn=document.getElementById("draw-btn"); btn.style.background=""; btn.style.borderColor="";
  if (tempRect) { map.removeLayer(tempRect); tempRect=null; }
  const s=Math.min(drawStart.lat,drawEnd.lat), n=Math.max(drawStart.lat,drawEnd.lat);
  const w=Math.min(drawStart.lng,drawEnd.lng), eo=Math.max(drawStart.lng,drawEnd.lng);
  if (Math.abs(n-s)<.001&&Math.abs(eo-w)<.001) { drawStart=null; return; }
  if (bboxRect) map.removeLayer(bboxRect);
  bboxRect=L.rectangle([[s,w],[n,eo]],{color:"#38b6e8",weight:1.5,fillColor:"#38b6e8",fillOpacity:.06,dashArray:"5 4"}).addTo(map);
  map.fitBounds([[s,w],[n,eo]],{padding:[30,30],animate:true,duration:.5});
  document.getElementById("bb-slat").value=s.toFixed(4); document.getElementById("bb-wlng").value=w.toFixed(4);
  document.getElementById("bb-nlat").value=n.toFixed(4); document.getElementById("bb-elng").value=eo.toFixed(4);
  showBboxResult(countInBox(s,w,n,eo),"vessel(s) inside drawn box");
  drawStart=null;
});

function runBbox() {
  const sl=parseFloat(document.getElementById("bb-slat").value), wl=parseFloat(document.getElementById("bb-wlng").value);
  const nl=parseFloat(document.getElementById("bb-nlat").value), el=parseFloat(document.getElementById("bb-elng").value);
  if ([sl,wl,nl,el].some(isNaN)) { alert("Please fill all four coordinate fields."); return; }
  const s=Math.min(sl,nl), n=Math.max(sl,nl), w=Math.min(wl,el), eo=Math.max(wl,el);
  if (bboxRect) map.removeLayer(bboxRect);
  bboxRect=L.rectangle([[s,w],[n,eo]],{color:"#38b6e8",weight:1.5,fillColor:"#38b6e8",fillOpacity:.06,dashArray:"5 4"}).addTo(map);
  map.fitBounds([[s,w],[n,eo]],{padding:[30,30],animate:true,duration:.5});
...
function countInBox(s,w,n,eo) {
  let c=0;
  for (let i=0;i<allVessels.length;i++) { const [lng,lat]=allVessels[i].geometry.coordinates; if(lat>=s&&lat<=n&&lng>=w&&lng<=eo) c++; }
  return c;
}
...
["bb-slat","bb-wlng","bb-nlat","bb-elng"].forEach(id => document.getElementById(id).addEventListener("keydown", e => { if(e.key==="Enter") runBbox(); }));

// ── EVENTS ───────────────────────────────────────────────────────────
document.querySelectorAll(".fchip").forEach(chip => chip.addEventListener("click", () => {
  document.querySelectorAll(".fchip").forEach(c=>c.classList.remove("active"));
  chip.classList.add("active"); activeFilter=chip.dataset.f; applyFilter();
}));
document.getElementById("search-inp").addEventListener("input", e => {
  clearTimeout(sDeb); sDeb=setTimeout(()=>{ searchQuery=e.target.value; applyFilter(); },200);
});
